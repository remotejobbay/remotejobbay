...
const JOB_URLS = [
  'https://job-boards.greenhouse.io/iovancebiotherapeutics/jobs/5147236008',
  'https://job-boards.greenhouse.io/iovancebiotherapeutics/jobs/5168511008',
  'https://job-boards.greenhouse.io/iovancebiotherapeutics/jobs/5171225008',
  'https://job-boards.greenhouse.io/iovancebiotherapeutics/jobs/5159170008',
];
...
async function main() {
  ...
  for (const url of JOB_URLS) {
    ...
    if (!parsed) throw new Error(`Could not parse Greenhouse job URL: ${url}`);

    const { company, jobId } = parsed;
    const job = await fetchJob(company, jobId);

    const title = job.title || '';
    const applyUrl = job.absolute_url || url;
    ...
    const jobData = {
      ...
      apply_url: applyUrl,
      logo: 'https://logo.clearbit.com/iovance.com',
